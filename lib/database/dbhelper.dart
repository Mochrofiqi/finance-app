...
class DBHelper {
  ...
  void _onCreate(Database db, int version) async {
    await db.execute(
      'CREATE TABLE cashflow(id INTEGER PRIMARY KEY, date STRING, nominal INTEGER, note TEXT, tipe STRING)',
    );
    ...
    await db.rawInsert(
      "INSERT INTO user (name, username, password) VALUES ('Moch. Rofiqi', 'mochrofiqi', 'password')",
    );
    print('DB Created');
  }
  ...
  Future<String> getPassword() async {
    ...
    List<Map> list = await dbClient!.rawQuery(
      "SELECT password FROM user WHERE username = 'mochrofiqi'",
    );
  ...
  Future<bool> updatePassword(String password) async {
    ...
    int res = await dbClient!.rawUpdate(
      "UPDATE user SET password = '$password' WHERE username = 'mochrofiqi'",
    );
